...
- **Single-genome scan** — *"does **this** genome carry my gene, and where?"* Build (or supply) an HMM and scan **one** genome — a local FASTA **or an NCBI accession** (fetched for you):
  ...
  ```
  Reports present / not-detected, the hit's coordinates + sequence, ORF validation, the **flanking genes** as an ordered `scan_neighbourhood.csv` relative to your gene — named from the **genome's own annotation** (gene/gp number, product, locus_tag, protein_id) for annotated records, or called with Prodigal (the discovery gene-caller) for plain FASTA — and (with `--find-interrupted`) interrupted/overprinted copies **plus the overprinting verdict**. Exit code 0 = found, 1 = absent (handy in loops over many genomes). Interactive: `bash start.sh` → mode 3. See [USAGE Case 11](docs/USAGE.md).
