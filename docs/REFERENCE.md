...
11. **Canonical run selection** — `_best_run_index` picks the run with the most validated hit rows (ties → earliest), matching `export_csv.py`'s `best_run`. All downstream figures, the published HMM, controls, and the paper table describe this same run.
